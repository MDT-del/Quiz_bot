...
def verify_payment():
    ...
    if status == 'OK':
        ...
        try:
            # تایید پرداخت با کتابخانه جدید
            verification_result = gateway.verify_payment_status(
                price=amount,
                auth_token=authority
            )

            if verification_result.get("is_successful"):
                user_id = payment_record['user_id']
                duration = 30 # یا هر مدتی که در نظر دارید
                ref_id = verification_result.get('reference_id', 'N/A')
                ...
                error_message = verification_result.get("error", "خطای نامشخص")
                update_payment_status(authority, 'verification_failed')
                return f"<h1>خطا در تایید پرداخت: {error_message}</h1>"
